...
// 1. Fix the Language Dropdown in Navbar
const fixedNavDropdown = `      <div class="lang-dropdown" id="langDropdown">
...
content = content.replace(/<div class="lang-dropdown" id="langDropdown">[\s\S]*?<\/div>\s*<\/div>/, fixedNavDropdown);

// 2. Cut content up to reviewModal form and rewrite the clean script block
const modalEndIdx = content.indexOf('<div class="modal-backdrop" id="reviewModal"');
if (modalEndIdx === -1) {
  console.error("Could not find reviewModal");
  process.exit(1);
}

const beforeModal = content.substring(0, modalEndIdx);

const completeScriptAndModals = `<!-- MODAL: WRITE REVIEW -->
<div class="modal-backdrop" id="reviewModal" onclick="closeModalOnBackdrop(event, 'reviewModal')">
  <div class="modal-card">
    ...
    <div class="form-msg form-error-msg" id="reviewError"></div>
...
<script>
  ...
  const TRANSLATIONS = {
    ...
    te: {
      ...
      mobile_btn_play: "📱 Play Store నుండి యాప్‌ను డౌన్‌లోడ్ చేసుకోండి (v5.5.0)",
  ...
  function updateUserAuthState() {
    // Keep header Sign In & Create Account buttons permanent
  }
  ...
  async function handleSignupSubmit(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          alert(data.message || 'Account created! Verification code sent to email.');
  ...
  function handleGoogleReviewAuth() {
    closeModal('reviewLoginRequiredModal');
    openModal('loginModal');
  }
        ...
        if (data.reviews.length === 0) {
          grid.innerHTML = \`
            <div style="grid-column: 1 / -1; text-align: center; padding: 3rem 2.5rem; background: #fff; border-radius: var(--radius-xl); border: 1.5px dashed var(--border); box-shadow: var(--shadow-hero);">
              ...
              <p style="font-size: .95rem; color: var(--text-muted); max-width: 500px; margin: 0 auto 1.5rem;">Sign in to your HisabHero account or Google to share your experience with thousands of businesses across India.</p>
  ...
  async function loadPublicStats() {
    try {
      ...
      // Keep existing default fallback
    }
  }
  ...
  // Initialize Cinematic Motion System & Saved Language on Load
...
const newIndexHtml = beforeModal + completeScriptAndModals;
fs.writeFileSync(indexPath, newIndexHtml, 'utf8');
console.log("Successfully rebuilt index.html with clean UTF-8 translations and working language switcher!");
